                ...
                {order.items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      {(item.book_file_url || item.book_file) && <a href={item.book_file_url || mediaUrl(`/uploads/books/${item.book_file}`)} target="_blank" rel="noopener noreferrer" className="text-orange inline-flex items-center gap-1" title="View PDF"><Eye size={18} /> View</a>}
